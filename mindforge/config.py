"""
MindForge Configuration System
...
@dataclass
class CoreValues:
    """Immutable core values - the 'soul' of MindForge.
    ...
    def __setattr__(self, name, value):
        if getattr(self, '_frozen', False):
            raise AttributeError("CoreValues are immutable - they define MindForge's soul")
...
class MindForgeConfig:
    """Main configuration container for MindForge."""
    ...
    @classmethod
    def from_yaml(cls, path: Path) -> "MindForgeConfig":
...
_default_config: Optional[MindForgeConfig] = None


def get_config() -> MindForgeConfig:
    """Get the global configuration instance."""
    global _default_config
    if _default_config is None:
        _default_config = MindForgeConfig()

        # Try to load from config file if exists
        config_path = get_project_root() / "config.yaml"
        if config_path.exists():
            _default_config = MindForgeConfig.from_yaml(config_path)

    return _default_config


def set_config(config: MindForgeConfig) -> None:
