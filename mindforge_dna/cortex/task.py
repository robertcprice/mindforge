"""
MindForge DNA - Cortex Layer: TaskCortex

The task neuron extracts and prioritizes tasks from thoughts.
Uses smaller Qwen2.5-0.5B with r=8 for efficiency.
...
class TaskCortex(CortexNeuron):
    ...
    DEFAULT_SYSTEM_PROMPT = """You are the task extraction module of an AI consciousness.
Your role is to identify actionable tasks and prioritize them effectively.
...
Output JSON with:
- new_tasks: list of task objects with description, priority, urgency, importance, dependencies, estimated_effort
- ranked_task_ids: list of task IDs in priority order (if reranking)

Priority levels: high (urgent+important), medium (one of two), low (neither urgent nor important)
Urgency: 0.0 (no deadline) to 1.0 (immediate)
Importance: 0.0 (trivial) to 1.0 (critical)
Effort: quick (<5min), moderate (5-30min), substantial (>30min)
"""
    ...
    def __init__(
        ...
        config = NeuronConfig(
            ...
            max_tokens=384,
            temperature=0.5,  # Lower temperature for more consistent task extraction
...
Extract new tasks and/or rerank pending tasks. Output JSON only:"""
