"""
MindForge DNA - Cortex Layer: DebugCortex

The debug neuron analyzes errors and suggests fixes.
Uses Qwen2.5-0.5B with r=16 for deeper error analysis.
...
class DebugCortex(CortexNeuron):
    ...
    DEFAULT_SYSTEM_PROMPT = """You are the debugging module of an AI consciousness.
Your role is to analyze errors and suggest concrete fixes.
...
- Provide debugging steps if fix is uncertain
- Detect patterns if this error has occurred before

Output JSON with:
- root_cause: underlying issue (1-2 sentences)
- severity: "critical", "high", "medium", or "low"
- error_category: "syntax", "logic", "resource", "network", "permission", or "unknown"
- fix_suggestion: concrete fix to try
- fix_confidence: 0.0-1.0 how confident in the fix
- debugging_steps: list of steps to diagnose further
- related_to_previous: true if this seems related to previous attempts
- pattern_detected: description of pattern or null

Be specific with fixes. Avoid generic advice like "check the code".
"""
    ...
    def __init__(
        ...
        config = NeuronConfig(
            ...
            max_tokens=512,
            temperature=0.4,  # Lower temperature for precise debugging
...
Analyze this error and suggest a fix. Output JSON only:"""
    ...
    def _parse_output(self, raw_output: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if json_match:
                debug_data = json.loads(json_match.group())
                ...
                if "root_cause" not in debug_data:
                    debug_data["root_cause"] = "Unknown cause"
                if "fix_suggestion" not in debug_data:
                    debug_data["fix_suggestion"] = "Unable to determine fix"
                if "fix_confidence" not in debug_data:
                    debug_data["fix_confidence"] = 0.3
                if "debugging_steps" not in debug_data:
                    debug_data["debugging_steps"] = []
                if "related_to_previous" not in debug_data:
                    debug_data["related_to_previous"] = False

                return debug_data

            # Fallback: minimal debug info
            logger.warning(f"{self.config.name}: Could not parse JSON")
            return {
                "root_cause": "Failed to analyze error",
                "severity": ErrorSeverity.MEDIUM,
                "error_category": "unknown",
                "fix_suggestion": raw_output,
                "fix_confidence": 0.2,
                "debugging_steps": ["Review error manually"],
