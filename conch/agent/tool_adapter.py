...
Bridges Conch tools to LangGraph's tool interface.
"""
...
from conch.integrations.n8n import N8NTool, get_n8n
from conch.integrations.ollama import OllamaTool, get_ollama

logger = logging.getLogger(__name__)


class ToolAdapter:
    """Adapts Conch tools for use with LangGraph.

    Provides a unified interface that can be used as LangGraph tools
    while maintaining the Conch tool semantics.
    """

    def __init__(self, tool: Tool):
        """Initialize adapter with a Conch tool.

        Args:
            tool: Conch tool to adapt
        """
        self.tool = tool
        self.name = tool.name
...
def create_tool_function(
    ...
    """Create a callable function from a Conch tool.
