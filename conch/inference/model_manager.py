...
from conch.inference.base import (
    BackendType,
    GenerationResult,
    InferenceBackend,
    InferenceConfig,
...
class ModelManager:
    ...
    def _detect_best_backend(self) -> BackendType:
        ...
        if is_apple_silicon:
            ...
            from conch.inference.mlx_backend import is_mlx_available
            if is_mlx_available():
                logger.info("Selected MLX backend (Apple Silicon)")
                return BackendType.MLX

        # Try llama.cpp
        from conch.inference.llamacpp_backend import is_llamacpp_available
        if is_llamacpp_available():
            logger.info("Selected llama.cpp backend")
            return BackendType.LLAMACPP

        # Fallback to Transformers
        from conch.inference.transformers_backend import is_transformers_available
    ...
    def _create_backend(self, backend_type: BackendType) -> InferenceBackend:
        ...
        if backend_type == BackendType.MLX:
            from conch.inference.mlx_backend import MLXBackend
            return MLXBackend(self.config)

        elif backend_type == BackendType.LLAMACPP:
            from conch.inference.llamacpp_backend import LlamaCppBackend
            return LlamaCppBackend(self.config)

        elif backend_type == BackendType.TRANSFORMERS:
            from conch.inference.transformers_backend import TransformersBackend
...
# Default system prompt for Conch
...
def get_conch_inference(
    model_path: str = "models/fine_tuned/conch_qwen",
    adapter_path: str = None,
) -> Callable[[str], str]:
    """Get inference function with Conch personality.
    ...
    Returns:
        Inference function with Conch system prompt
    """
    return get_inference_function(
        model_path=model_path,
        adapter_path=adapter_path,
        system_prompt=CONCH_SYSTEM_PROMPT,
    )
