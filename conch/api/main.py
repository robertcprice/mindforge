"""
Conch API - Main Application

FastAPI application providing REST endpoints for the Conch consciousness engine.
"""
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Starting Conch API...")
    ...
    try:
        ...
        logger.info("Conch API ready")

    except Exception as e:
        logger.error(f"Failed to initialize Conch: {e}")
        raise

    yield

    # Cleanup
    logger.info("Shutting down Conch API...")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="Conch API",
        description="REST API for the Conch consciousness engine",
        version="1.0.0",
        lifespan=lifespan,
    )
...
async def chat(request: ChatRequest):
    """
    Send a message and get a response from Conch.
