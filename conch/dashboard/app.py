"""
Conch Dashboard - Streamlit Application

Interactive dashboard for monitoring and interacting with the Conch
...
Run with: streamlit run conch/dashboard/app.py
Or: python -m conch.dashboard
"""
...
def initialize_conch():
    """Initialize Conch components."""
    ...
    try:
        ...
        st.error(f"Failed to initialize Conch: {e}")
        return False
...
def render_overview():
    ...
    if not st.session_state.initialized:
        st.warning("Conch not initialized. Click 'Initialize' in the sidebar.")
        if st.button("Initialize Conch"):
            with st.spinner("Initializing..."):
                if initialize_conch():
                    st.success("Conch initialized successfully!")
                    st.rerun()
        return
    ...
    with col2:
        ...
        if recent:
            for mem in recent:
                with st.expander(f"{mem.memory_type.value}: {mem.content}"):
...
def render_chat():
    ...
    st.title("Chat with Conch")

    if not st.session_state.initialized:
        st.warning("Please initialize Conch first (go to Overview).")
        return
...
def render_memory():
    ...
    if not st.session_state.initialized:
        st.warning("Please initialize Conch first.")
        return
    ...
    with tab1:
        ...
        if memories:
            for mem in memories:
                with st.expander(f"[{mem.memory_type.value}] {mem.content}", expanded=False):
...
def render_thoughts():
    ...
    with col1:
        from conch.core.thought import ThoughtTrigger
