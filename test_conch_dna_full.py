#!/usr/bin/env python3
"""
Conch DNA Architecture - Comprehensive Test Suite
...
Reference: docs/CONCH_DNA_FINAL_ARCHITECTURE.md
"""
...
# -----------------------------------------------------------------------------
# Global logging (tee prints -> file to preserve full LLM transcripts)
# -----------------------------------------------------------------------------
LOG_DIR = Path("logs")
LOG_DIR.mkdir(exist_ok=True)
ARTIFACT_DIR = Path("artifacts")
ARTIFACT_DIR.mkdir(exist_ok=True)
LOG_FILE = LOG_DIR / f"test_conch_dna_full_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"


class Tee:
    """Simple tee to mirror stdout/stderr into a log file."""
...
def extract_first_json(text: str) -> Optional[str]:
    ...
    for match in re.finditer(r'\{', text):
        try:
            _, end = JSON_DECODER.raw_decode(text[match.start():])
...
def test_superego_kvrm():
    ...
    from conch_dna.superego.kvrm import KVRMRouter, ClaimType
    import tempfile
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"  '{claim}' -> {actual_type.value}")
...
def test_superego_combined():
    ...
    from conch_dna.superego import SuperegoLayer
...
def test_ego_model():
    ...
    config = EgoConfig(
        ...
        max_tokens=1024,  # Keep runtime uncapped; bound test to avoid runaway loops
        temperature=0.7,
    )
    ...
    try:
        ...
        # Task-style generation: design note with strict bullet format
        response = ego.generate(
            "Respond with exactly 5 bullets (each starts with '- ') describing a file-listing CLI tool: cover requirements, CLI flags, error handling, logging, and tests. Do not include any other text before or after the bullets. Stop after the 5 bullets."
        )
        assert response and len(response) > 0, "EGO should generate response"
        bullet_lines = [line.strip() for line in response.splitlines() if line.strip().startswith("-")]
        assert len(bullet_lines) >= 5, "EGO should return at least 5 bullet lines"
        trimmed = "\n".join(bullet_lines[:5])
        print(f"  Generated bullets:\n{trimmed}")

        # Save artifact for inspection
        artifact_path = ARTIFACT_DIR / "ego_design.txt"
        artifact_path.write_text(trimmed)
        print(f"  Saved EGO artifact to {artifact_path}")
...
def test_cortex_neurons():
    ...
    from conch_dna.cortex.base import CortexNeuron

    neurons_tested = []

    # Test 1: THINK CORTEX (Qwen3-4B + LoRA r=16)
    think = ThinkCortex()
    ...
    out = think.think(
        context="Explain the architecture briefly",
        needs={},
        memories=[],
        recent_actions=[]
    )
    thought_json = extract_first_json(out.content)
    thought_obj = json.loads(thought_json) if thought_json else out.metadata
    cleaned = json.dumps(thought_obj, indent=2)
    print(f"    Think output:\n{cleaned}")
    (ARTIFACT_DIR / "think_thought.txt").write_text(cleaned)
    neurons_tested.append("Think")

    # Test 2: TASK CORTEX (Qwen3-1.7B + LoRA r=8)
    task = TaskCortex()
    ...
    task_out = task.infer({
        "thought": "We need to produce an architecture explainer, gather logs from the last run, and draft a changelog entry. Create actionable tasks with dependencies.",
        "pending_tasks": [],
        "context": "Prioritize anything that unblocks documentation and testing."
    })
    json_text = extract_first_json(task_out.content)
    task_obj = json.loads(json_text) if json_text else task_out.metadata
    assert task_obj.get("new_tasks"), "TaskCortex returned no tasks"
    cleaned = json.dumps(task_obj, indent=2)
    print(f"    Task output:\n{cleaned}")
    (ARTIFACT_DIR / "task_plan.json").write_text(cleaned)
    neurons_tested.append("Task")

    # Test 3: ACTION CORTEX (Qwen3-1.7B + LoRA r=8)
    action = ActionCortex()
    ...
    action_out = action.infer({
        "task": {"description": "inspect repository structure for architecture docs"},
        "available_tools": [
            {"name": "list_files", "description": "List files in a directory", "arguments": {"path": "string"}},
            {"name": "read_file", "description": "Read a text file", "arguments": {"path": "string"}}
        ],
        "context": "Use a tool call if possible; prefer list_files on docs/ first."
    })
    json_text = extract_first_json(action_out.content)
    action_obj = json.loads(json_text) if json_text else action_out.metadata
    assert action_obj.get("action_type"), "ActionCortex missing action_type"
    cleaned = json.dumps(action_obj, indent=2)
    print(f"    Action output:\n{cleaned}")
    (ARTIFACT_DIR / "action_plan.json").write_text(cleaned)
    neurons_tested.append("Action")

    # Test 4: REFLECT CORTEX (Qwen3-1.7B + LoRA r=8)
    ...
    reflect_out = reflect.infer({
        "cycle_events": ["Captured task plan", "Selected tool"],
        "needs_state": {},
        "outcomes": {"success": True},
        "task": "Compile architecture summary",
        "action": "Planned file listing and doc read",
        "result": "Plans generated"
    })
    json_text = extract_first_json(reflect_out.content)
    reflect_obj = json.loads(json_text) if json_text else reflect_out.metadata
    cleaned = json.dumps(reflect_obj, indent=2)
    print(f"    Reflect output:\n{cleaned}")
    (ARTIFACT_DIR / "reflect_note.json").write_text(cleaned)
    neurons_tested.append("Reflect")

    # Test 5: DEBUG CORTEX (Qwen3-1.7B + LoRA r=16)
    debug = DebugCortex()
    ...
    debug_out = debug.infer({"error": "stacktrace", "task": "fix bug", "previous_attempts": [], "context": "testing"})
    json_text = extract_first_json(debug_out.content)
    debug_obj = json.loads(json_text) if json_text else debug_out.metadata
    cleaned = json.dumps(debug_obj, indent=2)
    print(f"    Debug output:\n{cleaned}")
    (ARTIFACT_DIR / "debug_report.json").write_text(cleaned)
    neurons_tested.append("Debug")

    # Test 6: MEMORY CORTEX (Qwen3-1.7B + LoRA r=16)
    memory = MemoryCortex()
    ...
    memory_out = memory.infer({"query": "test memory", "memories": [], "context": "testing persistence of reflections"})
    json_text = extract_first_json(memory_out.content)
    memory_obj = json.loads(json_text) if json_text else memory_out.metadata
    cleaned = json.dumps(memory_obj, indent=2)
    print(f"    Memory output:\n{cleaned}")
    (ARTIFACT_DIR / "memory_eval.json").write_text(cleaned)
...
def test_memory_system():
    ...
    from conch_dna.memory.store import MemoryStore, Memory
    import tempfile
...
def test_training_pipeline():
    ...
    from conch_dna.training.pipeline import TrainingPipeline, TrainingExample, ExperienceBuffer
    import tempfile
    from datetime import datetime
...
def test_consciousness_loop():
    ...
    try:
        from conch_dna.main import ConsciousnessLoop, CycleState
    except ImportError as e:
...
def main():
    print("\n" + "="*70)
    print("CONCH DNA ARCHITECTURE - COMPREHENSIVE TEST SUITE")
    print("="*70)
    print("Testing against: docs/CONCH_DNA_FINAL_ARCHITECTURE.md")
    print("="*70)
