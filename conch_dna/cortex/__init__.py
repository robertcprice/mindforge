...
Neurons:
    - ThinkCortex: Thought generation and reasoning (Qwen3-4B, r=16)
    - TaskCortex: Task extraction and prioritization (Qwen3-1.7B, r=8)
    - ActionCortex: Tool selection and call formatting (Qwen3-1.7B, r=8)
    - ReflectCortex: Reflection and journaling (Qwen3-1.7B, r=8)
    - DebugCortex: Error analysis and fix suggestions (Qwen3-1.7B, r=16)
...
Usage:
    from conch_dna.cortex import ThinkCortex, TaskCortex
...
def create_cortex_suite(
    ...
    suite = {
        "think": create_think_cortex(
            base_model=base_models.get("think", "mlx-community/Qwen3-4B-4bit"),
            adapter_path=adapter_paths.get("think")
        ),
        "task": create_task_cortex(
            base_model=base_models.get("task", "mlx-community/Qwen3-1.7B-4bit"),
            adapter_path=adapter_paths.get("task")
        ),
        "action": create_action_cortex(
            base_model=base_models.get("action", "mlx-community/Qwen3-1.7B-4bit"),
            adapter_path=adapter_paths.get("action")
        ),
        "reflect": create_reflect_cortex(
            base_model=base_models.get("reflect", "mlx-community/Qwen3-1.7B-4bit"),
            adapter_path=adapter_paths.get("reflect")
        ),
        "debug": create_debug_cortex(
            base_model=base_models.get("debug", "mlx-community/Qwen3-1.7B-4bit"),
            adapter_path=adapter_paths.get("debug")
        ),
        "memory": create_memory_cortex(
            base_model=base_models.get("memory", "mlx-community/Qwen3-1.7B-4bit"),
