...
This module implements the EGO (Executing Generative Oracle) component of Conch DNA.
...
class EgoConfig:
    ...
    max_tokens: int = 1024  # Capped to prevent infinite generation
    temperature: float = 0.7
    personality_version: str = "v1"
    top_p: float = 0.95
    repetition_penalty: float = 1.3  # Increased to prevent repetition loops
...
Your Identity:
- Created through the Conch DNA architecture combining symbolic reasoning and neural learning
...
class EgoModel:
    ...
    def load(self, use_stub_on_failure: bool = True) -> bool:
        """Load the MLX model into memory.

        Args:
            use_stub_on_failure: If True, fall back to stub responses when model load fails.

        Returns:
            True if a real model is loaded, False if using stub or load failed.
        """
        if self._is_loaded:
            return True
        ...
        try:
            ...
            if use_stub_on_failure:
                logger.warning(
                    "EGO load failed; enabling stub responses instead of raising: %s", e
                )
                self._use_stub = True
                self.model = None
                self.tokenizer = None
        ...
        return self._is_loaded and not self._use_stub
    ...
    def _generate_text(
        ...
        # Use config max_tokens if not explicitly provided
        if max_tokens is None:
            max_tokens = self.config.max_tokens
        temperature = temperature or self.config.temperature

        # Default stop tokens prevent repetition loops
        default_stops = [
            "<|endoftext|>",
            "<|im_end|>",
            "\n\nUser:",
            "\n\nEcho:",  # Prevent "Echo:" repetition
            "\nEcho:",
            "Echo:\n",
            "\n\n\n",  # Triple newline indicates section break
        ]
        stop_tokens = stop_tokens or default_stops
        ...
        try:
            ...
            # Try streaming tokens (for live visibility); fall back if unsupported
            try:
                chunks = []
                token_counter = 0
                ...
                for resp in stream_generate(
                    ...
                    if any(current.endswith(tok) for tok in stop_tokens):
                        for tok in stop_tokens:
                            if current.endswith(tok):
                                current = current[:-len(tok)]
                                break
                        response = current
                        stop_reached = True
                        logger.info("[EGO] stop token reached")
                        break

                    # Detect repetition: if last 100 chars repeat 3+ times, stop
                    if len(current) > 300:
                        last_segment = current[-100:]
                        if current.count(last_segment) >= 3:
                            logger.warning("[EGO] Repetition detected, stopping generation")
                            # Trim to first occurrence
                            first_idx = current.find(last_segment)
                            response = current[:first_idx + len(last_segment)]
                            stop_reached = True
                            break

                    # Hard limit on total output
                    if len(current) > 4000:
                        logger.warning("[EGO] Output limit reached (4000 chars)")
                        response = current
                        stop_reached = True
                        break
                ...
                if stop_reached:
                    response = current if 'response' not in dir() else response
            except Exception:
                response = generate(
                    self.model,
                    self.tokenizer,
    ...
    def generate(
        ...
        Args:
            ...
            cycle_count: Number of wake cycles (influences energy/focus). Defaults to 0.
            mood: Current emotional state. Defaults to "neutral".
            dominant_need: Dominant drive from ID layer. Defaults to "curiosity".
        ...
        dominant_need = dominant_need or "curiosity"
