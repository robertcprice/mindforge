#!/usr/bin/env python3
"""
MindForge Local LoRA Training Script

Train MindForge personality and values using MLX on Apple Silicon.
Optimized for M4 Pro with 24GB RAM.

Usage:
    python scripts/train_mindforge.py [--model MODEL] [--epochs N] [--lr LR]

Examples:
    # Quick test run (4-bit, fast)
    python scripts/train_mindforge.py --model mlx-community/Qwen2.5-7B-Instruct-4bit --epochs 1

    # Full training (8-bit, better quality)
    python scripts/train_mindforge.py --model mlx-community/Qwen2.5-7B-Instruct-8bit --epochs 3
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train MindForge LoRA adapter")
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        "--output", default="./models/mindforge_trained", help="Output directory"
    )
    parser.add_argument(
    ...
    print(f"=" * 60)
    print(f"MindForge LoRA Training")
    print(f"=" * 60)
